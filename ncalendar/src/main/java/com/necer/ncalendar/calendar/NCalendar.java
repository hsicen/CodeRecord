...
public class NCalendar extends FrameLayout implements NestedScrollingParent, ValueAnimator.AnimatorUpdateListener,
        OnWeekCalendarChangedListener, OnMonthCalendarChangedListener,
        OnClickWeekCalendarListener, OnClickMonthCalendarListener {
    ...
    private void init() {
        ...
        weekHeigh = (int) (monthHeigh / 5 + Utils.dp2px(mContext, 15));
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (top == monthHeigh) {
                    ...
                    if (onCalendarChangedListener != null) {
                        onCalendarChangedListener.onChangeToMonthView();
                    }
                } else {
                    STATE = WEEK;
                    ...
                    if (onCalendarChangedListener != null) {
                        onCalendarChangedListener.onChangeToWeekView();
                    }
                }
            }
    ...
    public void onWeekCalendarChanged(DateTime dateTime) {
        try {
            if (STATE == WEEK) {
                ...
                requestLayout();
                if (onCalendarChangedListener != null) {
                    onCalendarChangedListener.onCalendarPageChanged(dateTime);
                }
            }
        } catch (Exception e) {
            Log.e(">>>", "Week日历出错", e);
    ...
    public void onClickWeekCalendar(DateTime dateTime) {
        monthCalendar.setDateTime(dateTime);
        if (onCalendarChangedListener != null) {
            onCalendarChangedListener.onClickCalendar(dateTime);
        }
    }
    ...
    public DateTime getSelectDate() {
        ...
        return STATE == MONTH ? monthCalendar.getSelectDateTime()
                : weekCalendar.getSelectDateTime();
    }

    /*** 设置是否显示农历*/
    public void isShowLunar(boolean isShow) {
        weekCalendar.isShowLunar(isShow);
        monthCalendar.isShowLunar(isShow);

        if (isShow) {
            setCalendarHeight((int) Utils.dp2px(mContext, 340));
        } else {
            setCalendarHeight((int) Utils.dp2px(mContext, 300));
        }
    }

    /*** 设置是否是日程的日历*/
    public void isSchedule(boolean isSchedule) {
        weekCalendar.isSchedule(isSchedule);
        monthCalendar.isSchedule(isSchedule);
    }

    /*** 设置一周的起始日为周一(1) 还是周天(0)*/
    public void setFirstDayOfWeek(int firstDayOfWeek) {
        weekCalendar.setFirstDayOfWeek(firstDayOfWeek);
        monthCalendar.setFirstDayOfWeek(firstDayOfWeek);
    }

    /*** 设置日历的高度*/
    public void setCalendarHeight(int height) {
        monthCalendar.setMonthCalendarHeight(height);
        weekCalendar.setMonthCalendarHeight(height);
    ...
    public void setWorkday(List<String> workday) {
        monthCalendar.setWorkdayList(workday);
        weekCalendar.setWorkdayList(workday);
    }

    /*** 设置休息数据*/
    public void setHoliday(List<String> holiday) {
        monthCalendar.setHolidayList(holiday);
        weekCalendar.setHolidayList(holiday);
    }

    /*** 设置事件项数*/
    public void setEventList(Map<String, String> eventList) {
        monthCalendar.setEventList(eventList);
        weekCalendar.setEventList(eventList);
    }
