...
public class CheckView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCountable) {
            ...
            if (mChecked) {
                ...
                initUnCheckedPaint();
                canvas.drawCircle((float) SIZE * mDensity / 2, (float) SIZE * mDensity / 2,
                        BG_RADIUS * mDensity, mUnCheckPaint);
            }
        }
    ...
    private void initBackgroundPaint() {
        if (mBackgroundPaint == null) {
            ...
            int defaultColor = ResourcesCompat.getColor(
                    getResources(), R.color.colorAccent,
                    getContext().getTheme());
            int color = ta.getColor(0, defaultColor);
            ta.recycle();
    ...
    private void initUnCheckedPaint() {
        if (mUnCheckPaint == null) {
            mUnCheckPaint = new Paint();
            mUnCheckPaint.setAntiAlias(true);
            mUnCheckPaint.setStyle(Paint.Style.FILL);

            int defaultColor = ResourcesCompat.getColor(
                    getResources(), R.color.colorUnCheck,
                    getContext().getTheme());

            mUnCheckPaint.setColor(defaultColor);
        }
    }
